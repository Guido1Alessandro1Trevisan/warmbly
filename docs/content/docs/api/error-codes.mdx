...
One conflict carries its own `code`. A contact's email address is unique within the workspace, so changing one to an address another contact already holds is refused rather than merging the two:

| `code` | Status | Meaning |
|--------|--------|---------|
| `contact_email_taken` | 409 | The address given to [update a contact](/api/reference/contacts/#update-a-contact) belongs to another contact in the workspace |
