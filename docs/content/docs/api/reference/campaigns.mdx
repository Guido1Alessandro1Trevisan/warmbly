...
| `steps` | object[] | no | Initial sequence steps in order (see create sequence input below). They are connected in order: each step routes unconditionally to the next, waiting that step's `wait_after` days. The first step's `wait_after` defaults to `0`, follow-ups to `3`. Every step defaults to `thread_reply: true`, so a follow-up is sent as a reply on the conversation the first email opened and carries its subject; pass `thread_reply: false` on a step that should open a new one with its own subject. |
