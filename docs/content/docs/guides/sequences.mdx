...
<Callout type="info" title="Threading needs the first email to have been confirmed sent">
The reply headers point at the message id the sending provider put on the wire, which Warmbly learns when the worker reports the send. If that confirmation never arrives, the follow-up still sends, as its own conversation.
</Callout>
