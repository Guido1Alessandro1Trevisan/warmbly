...
The email address can be changed here, which is the right move when someone's address was mistyped on import or they moved to a new domain. It is stored lowercased, and it has to be free: an address another contact in the workspace already holds is refused, because merging two people's campaign history is not something the edit could undo. A new address also clears the contact's [verification](/guides/deliverability/#address-verification) verdict, since the old one described a different mailbox, and the background check picks the new address up on its next pass. Emails already sent went to the old address and stay in the timeline as they happened.
