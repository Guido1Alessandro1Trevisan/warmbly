...
| `BOX_OUTLOOK_TENANT_ID` | Optional Entra directory UUID used by both the backend authorization flow and worker token refresh. Set it for a single-tenant app; unset uses `/common` for an intentionally multi-tenant app | unset |
...
| Variable | What it does | Default |
|---|---|---|
| `EMAIL_VERIFY_BOUNCER_API_KEY` | Uses the instance's Bouncer account to verify recipients from the backend before campaign sending | unset |
| `EMAIL_VERIFY_STRICT` | When `true`, only Bouncer `deliverable` verdicts checked within the last 30 days may be sent. Risky, unknown, stale, manual, imported, and provider-error results remain held | `false` |

Strict verification fails closed. It does not fall back to the built-in SMTP probe when Bouncer is unavailable or out of credits, and the final campaign routing gate repeats the provider, source, verdict, and freshness checks before reserving work for a sender.
