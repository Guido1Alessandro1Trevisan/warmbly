...
| forms | 8090 | Hosted form pages, embeds and public submissions; `GET /health` |
...
| forms | No database and no event bus: only `BACKEND_INTERNAL_URL` and `INTERNAL_API_TOKEN` |
...
# For an app registered as "My organization only", use its directory UUID.
BOX_OUTLOOK_TENANT_ID=00000000-0000-0000-0000-000000000000
```
</Step>
...
For a single-tenant Entra registration, set `BOX_OUTLOOK_TENANT_ID` to the
directory UUID on both the backend and every worker. This pins authorization
and refresh to that tenant. Leave it unset only for an app intentionally
registered for multiple organizations; the default then remains Microsoft's
`/common` endpoint. A malformed nonempty value never falls back to `/common`.
...
<Callout type="warn" title="The redirect URI belongs to the API, not the dashboard">
...
| Sign in with Google | `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_REDIRECT_URI` (defaults to `<API_PUBLIC_URL>/v1/auth/google/callback`) |
...
| Error tracking | `POSTHOG_KEY` (plus `POSTHOG_HOST` for a PostHog you run) or `SENTRY_DSN`, and `WARMBLY_POSTHOG_KEY` / `WARMBLY_SENTRY_DSN` for the browser apps. Either, both or neither; optional in every environment, including `prod` |
...
No containers at all is also supported: every service is a static binary or a static build. [Deploying without Docker](/development/bare-metal/) builds them from source and runs them under systemd with nginx in front, using the units in `deploy/systemd/` and the site in `deploy/nginx/`.
